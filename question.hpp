...
private:
	...
	std::vector<std::string> options_words;
	...
	void read() {
		...
		if (!print_option) {
			...
			while (getline(ifile, str)) {
				...
				tmp.option_word = str;
				options_words.push_back(str);
				ifile >> tmp.available; ifile.get();
				getline(ifile, str);
				if (str != "") {
					ss << str;
					ss >> n, ss >> m;
...
public:
	Question(std::pair<int, int> info) : i(info.first), j(info.second) {
		read();
		//for (auto & i : options_words) std::cout << i << '\n';
	}
	std::pair<int, int> ask(std::pair<int, int> info) {
		print_question_content(question_name, options_words);
		bool ask_flag = false;
